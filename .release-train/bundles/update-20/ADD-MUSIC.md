...
5. Enter the title, artist, release type, genre, and any optional collection title, track number, franchise, mood, search tags, credits, explicit marking, description, YouTube link, or lyrics.
...
The public player displays complete `published` entries plus `scheduled` entries after their valid `releaseAt` time. Drafts and archived entries remain hidden. Optional `collection`, `trackNumber`, `releaseType`, `franchise`, `mood`, `tags`, `credits`, and `explicit` fields improve browsing and Now Playing details. Optional `description`, `releaseDate`, and an HTTPS `youtubeUrl` from youtube.com/youtu.be appear in the release information. An optional `lyrics` string enables the Lyrics control inside Now Playing; leaving it out keeps that control hidden.
