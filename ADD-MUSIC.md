...
5. Enter the title, artist, release type, genre, date, optional description, and optional lyrics.
...
The public player displays only entries with `status: "published"`, a valid MP3 path, a cover path, title, artist, and duration. An optional `lyrics` string enables the Lyrics control inside Now Playing; leaving it out keeps that control hidden.
